            ...
            with tf.variable_scope("action_value"):
                if feature_extraction == "cnn":
                    ...
                    for layer_size in layers:
                        action_out = layers.fully_connected(action_out, num_outputs=layer_size, activation_fn=None)
                        if layer_norm:
                            action_out = layers.layer_norm(action_out, center=True, scale=True)
                        action_out = act_fun(action_out)

                action_scores = layers.fully_connected(action_out, num_outputs=self.n_actions, activation_fn=None)

            if self.dueling:
                with tf.variable_scope("state_value"):
                    state_out = extracted_features
                    for layer_size in layers:
                        state_out = layers.fully_connected(state_out, num_outputs=layer_size, activation_fn=None)
                        if layer_norm:
