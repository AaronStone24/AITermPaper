...
from stable_baselines.common.tf_util import get_trainable_vars, total_episode_reward_logger


class DDQN(OffPolicyRLModel):
    """
    The DQN model class. This is a multiagent implementation of the DQN algorithm based on the single agent
    implementation in Stable Baselines: https://github.com/hill-a/stable-baselines
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=5e-4, buffer_size=50000, exploration_fraction=0.1,
                 ...
                 _init_setup_model=True, policy_kwargs=None, full_tensorboard_log=False, initial_exploration=1):

        super(DDQN, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose, policy_base=DQNPolicy,
    ...
    def _get_pretrain_placeholders(self):
        """
        Get pretrain placeholders function
        """
    ...
    def setup_model(self):
        ...
        with SetVerbosity(self.verbose):
            assert not isinstance(self.action_space, gym.spaces.Box), \
                "Error: DDQN cannot output a gym.spaces.Box action space."
            ...
            assert issubclass(test_policy, DQNPolicy), "Error: the input policy for the DDQN model must be " \
            ...
            with self.graph.as_default():
                ...
                self.params = get_trainable_vars("deepq")
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=100, tb_log_name="DQN",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            if self.prioritized_replay:
                ...
                self.beta_schedule = None
            # Create the schedule for exploration starting from initial_exploration.
            self.exploration = LinearSchedule(schedule_timesteps=int(self.exploration_fraction * total_timesteps),
                                              initial_p=self.initial_exploration,
            ...
            for _ in range(total_timesteps):
                ...
                with self.sess.as_default():
                    ...
                    for i in range(self.n_agents):
                        action[i] = self.act(np.array(obs[i])[None], update_eps=update_eps, **kwargs)[0]
                ...
                for i in range(self.n_agents):
                    self.replay_buffer.add(obs[i], action[i], rew[i], new_obs[i], float(done))
                obs = new_obs
    ...
    def predict(self, observations, states=None, mask=None, deterministic=True):
        """
        Predict function
        """
        ...
        with self.sess.as_default():
            for na in range(self.n_agents):
                observation = observations[na].reshape((-1,) + self.observation_space.shape)
                actions[na], _, _ = self.step_model.step(observation, deterministic=deterministic)
    ...
    def action_probability(self, observation, state=None, mask=None, actions=None):
        ...
        if actions is not None:  # comparing the action distribution, to given actions.
            ...
            actions_proba = actions_proba.reshape((-1, 1)) # normalize action proba shape.
